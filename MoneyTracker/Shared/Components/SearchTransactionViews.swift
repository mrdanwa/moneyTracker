...
struct SearchTransactionView: View {
    ...
    private var filteredTransactions: [Transaction] {
        store.transactions.filter { transaction in
            // First filter by current account
            ...
            // Add type filter
            let matchesType = selectedFilter == nil || transaction.type == selectedFilter
            
            return matchesAccount && matchesSearch && matchesDateRange && matchesType
        }
    }
